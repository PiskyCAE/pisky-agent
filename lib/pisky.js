...
class PiskyClient {
   ...
   *   type       {string} — buy_signal|sell_signal|rug_alert|momentum|insight
  ...
  async swarmInsights(limit = 20) {
    return this.call('swarm-insights', `?limit=${limit}`);
  }
  ...
  async taskPropose(agentId, address, opts = {}) {
    ...
    // If reward specified and escrow wallet available, deposit PISKY first
    let escrowTxSig = null;
    if (rewardPisky > 0 && this.wallet) {
      try {
        const quote = await this.getQuote();
        const escrowWallet = quote.escrowWallet;
        if (escrowWallet) {
          const amountRaw = BigInt(rewardPisky) * BigInt(1_000_000); // PISKY has 6 decimals
          log('info', `Depositing ${rewardPisky} PISKY to escrow for task reward`);
          ...
          log('warn', 'Escrow wallet not configured on server — proposing with honor-system reward');
        }
      } catch (err) {
