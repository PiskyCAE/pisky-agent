  ...
  try {
    switch (task.type) {
      ...
      case 'research': {
        ...
        // If the task involves holder concentration, fetch batch data for top pool mints
        const holderKeywords = ['holder', 'concentration', 'whale', 'distribution', 'top.*wallet'];
        const isHolderTask = holderKeywords.some(kw => new RegExp(kw, 'i').test(task.description + ' ' + task.title));
        if (isHolderTask && pools.status === 'fulfilled' && pools.value?.pools?.length) {
          const mints = pools.value.pools
            .slice(0, 10)
            .map(p => p.baseToken?.mint ?? p.mint)
            .filter(Boolean);
          if (mints.length) {
            try {
              const holderData = await api._fetch(`/api/token-holders-batch?mints=${mints.join(',')}`);
              if (holderData.ok) {
                const json = await holderData.json();
                sections.push('HOLDER CONCENTRATION (top pools, top 5/10/20% of supply):\n' + JSON.stringify(json.results, null, 2).slice(0, 4000));
              }
            } catch (e) {
