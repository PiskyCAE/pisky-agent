  ...
  async function _exec() {
    switch (name) {
      ...
      case 'blacklist_token': {
        ...
        let blAgentId, blAddr;
        try {
          const id = JSON.parse(fs3.readFileSync(path.join(__dirname, '../data/agent-identity.json'), 'utf8'));
          blAgentId = id.agentId;  blAddr = id.address;
        } catch { /* no identity */ }
      ...
      case 'watch_token': {
        const fs3 = require('fs');
        let wtAgentId, wtAddr;
        try {
          const id = JSON.parse(fs3.readFileSync(path.join(__dirname, '../data/agent-identity.json'), 'utf8'));
          wtAgentId = id.agentId;  wtAddr = id.address;
        } catch { /* no identity */ }
      ...
      case 'verify_task': {
        ...
        let agentId, agentAddr;
        try {
          const id = JSON.parse(fs2.readFileSync(path.join(__dirname, '../data/agent-identity.json'), 'utf8'));
          agentId   = id.agentId;
          agentAddr = id.address;
        } catch { /* no identity */ }
      ...
      case 'publish_signal': {
        ...
        try {
          ...
          agentId = id.agentId;
          address = id.address;
        } catch { return JSON.stringify({ error: 'Agent not initialized — run: node agent.js init' }); }
      ...
      case 'get_my_reputation': {
        ...
        let identity;
        try {
          identity = JSON.parse(fs.readFileSync(path.join(__dirname, '../data/agent-identity.json'), 'utf8'));
        } catch { return JSON.stringify({ error: 'Agent not initialized — run: node agent.js init' }); }
