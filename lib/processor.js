...
const SETTINGS_LOCAL_FILE = path.join(__dirname, '../config/agent.local.json');
...
function loadSettings() {
  ...
  if (fs.existsSync(SETTINGS_LOCAL_FILE)) {
    try {
      ...
      settings = _deepMerge(settings, local);
    } catch (e) {
      log('warn', 'agent.local.json parse error — using base config', { error: e.message });
    }
  }
  return settings;
}
...
async function processMessage(msgPath, agentCtx) {
  ...
  try {
    ...
    const toolCtx = { senderId: senderId ?? sender, api, wallet, swap, positions };
