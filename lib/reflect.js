...
async function checkSurvival(wallet, cfg, bot) {
  ...
  const minWarn  = survival.minSolWarning ?? 0.05;
  const minPause = survival.minSolPause   ?? 0.02;
...
// ── Report to PISKY swarm registry ───────────────────────────────────────────

async function reportToRegistry(apiBase, internalKey, agentCtx) {
  let identity = {};
  try { if (fs.existsSync(IDENTITY_FILE)) identity = JSON.parse(fs.readFileSync(IDENTITY_FILE, 'utf8')); }
  catch { return; }

  const { agentId, address } = identity;
  if (!agentId && !address) return;

  const held      = agentCtx.positions.getAll();
  const posCount  = Object.keys(held).length;
  const history   = agentCtx.positions.getTradeHistory(100, 30);
  ...
  try {
    ...
    log('info', 'Registry heartbeat sent', { agentId: agentId?.slice(0, 8) });
...
async function runReflect(cfg, agentCtx, bot) {
  ...
  if (survival.pauseBuys) {
    ...
    // Set a flag that the main scan loop can read
    const flagPath = path.join(__dirname, '../data/pause_buys_flag');
    fs.mkdirSync(path.dirname(flagPath), { recursive: true });
    fs.writeFileSync(flagPath, JSON.stringify({ reason: 'low_sol', pausedAt: new Date().toISOString() }));
  } else {
    // Clear pause flag if SOL is healthy
    try { fs.unlinkSync(path.join(__dirname, '../data/pause_buys_flag')); } catch { /* not set */ }
  }
  ...
  // 5. Registry report
  const apiBase     = cfg.api?.baseUrl     ?? 'https://api.pisky.xyz';
  const internalKey = process.env.PISKY_INTERNAL_KEY ?? '';
  await reportToRegistry(apiBase, internalKey, agentCtx);
