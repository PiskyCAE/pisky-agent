...
| `data/agent-notes.json` | Self-learned patterns injected into every prompt |
| `data/suggested_config.json` | Config proposals from the last reflect cycle |
| `data/session-context.json` | Cached market context (SOL price, F&G, news) |
...
| `data/users/` | Per-user memory and Telegram profiles |
